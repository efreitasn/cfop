...
To start parsing a CLI, just call the Init function passing the name and description of the root command,
which is the first term when executing a command, the os.Args slice and a parser.
...
and description to another Parser, which could be another SubcmdsSet, and doesn't accept any options or flags,
only subcommands. When parsing, the first parser called is the one provided to the Init function and any
subsequent parsers are called according to the terms (os.Args) until a Cmd is reached. After that, any options,
arguments and/or flags are parsed.
...
subcommand uses a Cmd parser, which has only one option, called name. The foo subcommand is a SubcmdsSet
parser, which means that it also has one or more subcommands. In this case, it has one subcommand named bar, which
is a 'Cmd'. Now, when run
